hostapd_set_bss_options() {
	local var="$1"
	local vif="$2"
	logger "chad- hostapd_set_bss_options() $1 $2"
...
hostapd_set_log_options() {
	local var="$1"
	local cfg="$2"
	logger "chad- hostapd_set_log_options() $1 $2"
	local log_level log_80211 log_8021x log_radius log_wpa log_driver log_iapp log_mlme

	config_get log_level "$cfg" log_level 2
	logger "chad- log_level=$log_level"
	...
	logger "chad- log_80211=$log_80211"
	logger "chad- log_8021x=$log_8021x"
	logger "chad- log_radius=$log_radius"
	logger "chad- log_wpa=$log_wpa"
	logger "chad- log_driver=$log_driver"
	logger "chad- log_iapp=$log_iapp"	
	logger "chad- log_mlme=$log_mlme"
...
hostapd_setup_vif() {
	local vif="$1"
	local driver="$2"
	logger "chad- hostapd_setup_vif() $1 $2"
	...
	config_get device "$vif" device
	logger "chad- device=$device"
	config_get channel "$device" channel
	logger "chad- channel=$channel"
	config_get hwmode "$device" hwmode
	logger "chad- hwmode=$hwmode"
...
EOF
	...
	logger "chad- hostapd -P /var/run/wifi-$ifname.pid -B /var/run/hostapd-$ifname.conf"
}
